 ...
 * a Pip. A turn was ten-plus free unit actions against one or two card plays, so the deck was
 * a garnish on a turn that was already full.
 *
 * The shape now: **a swing costs a Pip, and a body that does not swing makes one.** Every turn
 ...
 * overhaul is titled **The Pip Tax**. That was written about *bodies costing Pips to summon* —
 * a one-off purchase against a trickle, where "a 3-Pip ranged body is three turns of the entire
 * economy." This is a different shape: a **cycle**, where the warband funds itself and the
 * player allocates. The old complaint was that buying a board meant casting nothing; here a
 * board is what lets you cast at all.
 *
 ...
 *
 * | attacking with | net Pips/turn | reads as |
 * |---|---|---|
...
export const ATTACK_PIP_COST = 1;

/**
 * Pip income per turn, given how many bodies a side has standing.
 *
 * `2 + bodies/3` rather than a flat `1`. The third is what lets a large warband feed itself
 * without making it free — nine bodies earn five, against nine Pips to swing with all of them.
 *
 * **The floor is 2, and it was 1, and the difference is the endgame.** A fight that reaches one
 * body a side earns exactly one Pip and spends exactly one attacking with it, so it can never
 ...
 * against the player's one, which printed the enemy seven Pips a turn to the player's two and
...
export function pipIncomeFor(bodies: number): number {
...
export interface ChannelYield {
  ...
  /** Evaporates at end of turn. Unchanged from before: a strict cost Pips cannot cover. */
 ...
 * | | | |
 * |---|---|---|
 * | melee (2) | **+1 Pip** | the workhorse battery — cheap bodies are what you can afford to sit down |
 * | ranged (3) | **+1 card** | artillery feeds the hand, not the bank; the one class that answers a dead draw |
 * | elite (4) | **+2 Pips** | pays for itself, and is the reason to spend four points on one body |
...
export function channelYieldFor(def: CardDef): ChannelYield | null {
  switch (rosterPointsOf(def)) {
    case 2:
      return { pips: 1, marrow: 1, draw: 0, verb: 'Brace' };
    case 3:
      return { pips: 0, marrow: 1, draw: 1, verb: 'Sight' };
    case 4:
      return { pips: 2, marrow: 1, draw: 0, verb: 'Focus' };
    default:
